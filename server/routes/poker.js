...
router.post('/:gameId/action', async (req, res) => {
  try {
    ...
    if (game.status === 'playing' && 
      ...
      const randomDelay = Math.random() * 2000 + 2000; // 2-4 секунди (зменшено в 2 рази)
...
async function processBotAction(gameId) {
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 1500)); // 1.5 секунды базовой затримки (зменшено в 2 рази)
    console.log(`[BOT-ACTION] ⏰ Базовая затримка 1.5 секунды завершена`);
    ...
    if (activePlayers.length === 1) {
      ...
      // Запускаем следующую игру через 5 секунд
      setTimeout(() => {
        startNextGame(freshGame);
      }, 5000); // ИЗМЕНЕНО: увеличил с 3000 до 5000ms (5 секунд)
...
async function advanceToNextRound(game) {
  ...
  if (currentPlayer && currentPlayer.isBot && !currentPlayer.folded && !currentPlayer.hasActed) {
    ...
    }, 6000); // ИЗМЕНЕНО: увеличил с 1000 до 6000ms (6 секунд)
  } else {
